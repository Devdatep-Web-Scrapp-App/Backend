...
class InstagramScraperService:
    ...
    def __init__(self, app_user_id: int, ig_username: str, ig_password: str = None):
        ...
        # Directorio para guardar la sesion de instagrapi (JSON, no Chrome)
        self.session_dir  = os.path.join(os.getcwd(), "ig_sessions")
        os.makedirs(self.session_dir, exist_ok=True)
        self.session_file = os.path.join(self.session_dir, f"user_{self.app_user_id}.json")
    ...
    def _build_client(self) -> Client:
        cl = Client()
        cl.delay_range = [1, 3]  # simular comportamiento humano

        if os.path.exists(self.session_file):
            try:
                cl.load_settings(self.session_file)
                cl.login(self.ig_user, self.ig_password or "")
                cl.get_timeline_feed()  # verificar que la sesion es valida
        ...
        print(f"[{self.ig_user}] Login exitoso, sesion guardada.")
        return cl

    def setup_session(self):
        """
        Realiza el primer login y guarda la sesion en disco.
        Llamar desde el endpoint /scraper/setup-instagram.
        """
        if not self.ig_password:
            raise ValueError("Se requiere contraseña para setup.")
        try:
            ...
            raise ValueError("La cuenta tiene verificación en dos pasos activada. Desactívala temporalmente para el setup.")
        except ChallengeRequired:
            raise ValueError("Instagram requiere verificación adicional. Inicia sesion manualmente en la app y vuelve a intentarlo.")
        except Exception as e:
            raise ValueError(f"Error durante el setup: {str(e)}")

    def _tiene_sesion_guardada(self) -> bool:
    ...
    def run_extraction(self):
        ...
        try:
            # Obtener user_id de Instagram a partir del username
            user_id = cl.user_id_from_username(self.ig_user)
            print(f"[{self.ig_user}] User ID: {user_id}")

            # Obtener todos los seguidores paginados
            print(f"[{self.ig_user}] Obteniendo seguidores...")
            followers_raw = cl.user_followers(user_id, amount=0)  # amount=0 = todos
            ...
            # Sesion expirada durante el scraping — borrar y pedir re-setup
            if os.path.exists(self.session_file):
